...
registerMooseObject("panmooseApp", GmBinaryExtended);

InputParameters
GmBinaryExtended::validParams()
{
  ...
  params.addParam<std::string>("elements","Fe Cr","Elements to be defined. Specify two.");
...
GmBinaryExtended::~GmBinaryExtended()
  {
	if(_m_pfm_sdk != NULL){
...
GmBinaryExtended::GmBinaryExtended(const InputParameters & parameters)
...
// Components
	...
	vector<double> avg_comp = { 0.49, 0.51 };
...
// Set therm_set_temp for more than one phase
	...
	vector<double> conc_vector = { _pps_x1_avg, 1.0-_pps_x1_avg};
	...
	cout<<"Calculated global point: (x1,TK) :"<<"("<<_pps_x1_avg
												<<","<<TK<<")"<<endl;
	cout<<" Mobility M1         :"<<_Mob1<<endl;
	cout<<" Mobility M2         :"<<_Mob2<<endl;
	cout<<" Non-dimensional w1  :"<<(output.mu[0]-output.mu[1])/_Gnormal<<endl;
	cout<<" Non-dimensional d2G :"<< cal_d2G(output,0,0,0)/_Gnormal<<endl;
}//end of constructor

void
GmBinaryExtended::computeQpProperties()
{
  ...
  Real X1 = _X1[_qp];
  Real X2 = 1-X1;
  vector<double> conc_vector = {X1,X2};
  Real T  = _TK[_qp];
...
//Second derivative
  ...
  _d2GdX1X1[_qp] = cal_d2G(output,engine_id,0,0) / _Gnormal;
	...
	{
// Chemical mobility
	Real M1=_Mob1, M2=_Mob2;
	Real X1=_pps_x1_avg;
	Real X2=1.0-X1;
	_Mob11[_qp] = 1/_Vm*( X1*X2*(X1*M2+X2*M1) );
	}
