...
registerMooseObject("panmooseApp", MySplitCHParsed);

InputParameters
MySplitCHParsed::validParams()
{
  InputParameters params = SplitCHCRes::validParams();
  params.addClassDescription(
      "Personal: Split formulation Cahn-Hilliard Kernel that uses a DerivativeMaterial Free Energy");
...
MySplitCHParsed::MySplitCHParsed(const InputParameters & parameters)
...
Real
MySplitCHParsed::computeDFDC(PFFunctionType type)
{
  switch (type)
  {
    case Residual:
    ...
    case Jacobian:
			std::cout<<"Calling Jacobian,diag: "<<_d2Fdc2[_qp]<<std::endl;
...
Real
MySplitCHParsed::computeQpOffDiagJacobian(unsigned int jvar)
{
  if (jvar == _w_var)
